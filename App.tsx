...
const App: React.FC = () => {
  ...
  // Sync Global Profile whenever it changes
  useEffect(() => {
    if (localPlayerId && playerProfile.name) {
      ...
      localStorage.setItem('uno_inventory', JSON.stringify(profileInventory()));
      localStorage.setItem('uno_stats', JSON.stringify(playerProfile.stats));
    }
  }, [playerProfile]);

  const profileInventory = () => playerProfile.inventory || ["default_skin"];
  ...
  const calculateWinnerScore = (winnerId: string, allPlayers: Player[]): number => {
    ...
    return Math.max(score, 50); // Mínimo de 50 pontos por vitória
  };
  ...
  const joinRoom = (roomId: string) => {
    lobbyChannel?.send({ type: 'broadcast', event: 'player_joined_request', payload: { ...playerProfile, id: localPlayerId!, hand: [], isHost: false, isBot: false, hasCalledUno: false, roomId, score: 0 } });
    setCurrentView(AppView.GAME);
  };
