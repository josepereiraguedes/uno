...
const GameTable: React.FC<GameTableProps> = ({ gameState, localPlayerId, reactions, onPlayCard, onDrawCard, onCallUno, onSendReaction, onTimeout, onSendVoice, onStartGame, onLeaveRoom, onlinePlayers }) => {
  const localPlayer = gameState.players.find(p => p.id === localPlayerId)!;
  const isMyTurn = gameState.status === GameStatus.PLAYING && gameState.players[gameState.currentPlayerIndex].id === localPlayerId;
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      if (remaining === 0 && lastSecondRef.current !== 0) {
        ...
        onTimeout(gameState.players[gameState.currentPlayerIndex].id);
      }
    }, 500);
      ...
      <div className="flex justify-center items-end w-full max-w-full overflow-visible px-2 h-full">
        {hand.map((card, i) => {
          ...
          return (
            ...
            >
              <UnoCard card={card} size={screenWidth < 768 ? 'sm' : 'md'} playable={playable} disabled={!isMyTurn} />
            </div>
          );
        })}
      </div>
  ...
  const PlayerVisual = ({ p, isLocal }: { p: Player, isLocal?: boolean }) => {
    ...
    return (
      <div className={`relative flex flex-col items-center transition-all ${currentPlayer.id === p.id ? 'scale-110 z-50' : 'opacity-70 z-10'}`}>
        ...
        <div className="relative mb-1">
          <div className={`w-10 h-10 lg:w-20 lg:h-20 rounded-full border-4 ${currentPlayer.id === p.id ? 'border-yellow-400 shadow-[0_0_20px_rgba(250,204,21,0.6)] animate-pulse' : 'border-white/10'} bg-black/40 flex items-center justify-center text-xl lg:text-4xl`}>{p.avatar}</div>
  ...
  if (gameState.status === GameStatus.LOBBY) {
    ...
    const canStart = gameState.settings.mode !== GameMode.RANKED || humans >= 2;

    return (
        <div className="flex-1 flex flex-col lg:flex-row h-full animate-fade-in z-10 overflow-hidden relative">
          <button 
            onClick={onLeaveRoom}
            className="absolute top-4 left-4 z-[200] bg-white/5 hover:bg-red-600/20 text-white/40 hover:text-white px-4 py-3 rounded-2xl flex items-center gap-2 border border-white/10 transition-all font-black uppercase text-[10px] tracking-widest"
          >
            <span>🚪</span> ABANDONAR ARENA
          </button>

          <div className="flex-1 flex flex-col items-center justify-center p-6 space-y-8 bg-black/20">
            <div className="text-center">
              ...
              <p className="text-blue-400 font-black tracking-[0.4em] uppercase text-[10px] mt-2">Aguardando Desafiantes...</p>
              <div className="mt-4 inline-block bg-white/10 px-4 py-2 rounded-full border border-white/10 font-black text-[10px] tracking-widest uppercase">
                 MODO: <span className={gameState.settings.mode === GameMode.RANKED ? 'text-yellow-400' : 'text-blue-400'}>{gameState.settings.mode}</span>
              </div>
            </div>
            ...
            <div className="flex flex-wrap justify-center gap-3 max-w-2xl px-4 overflow-y-auto no-scrollbar max-h-[40vh]">
              {gameState.players.map(p => (
                <div key={p.id} className="bg-black/40 p-3 rounded-[2rem] border border-white/10 flex flex-col items-center gap-2 w-24 lg:w-36 animate-slide-up relative group">
                  ...
                  <div className="text-center">
                     <p className="font-bold text-[8px] lg:text-xs truncate w-20 lg:w-28 uppercase tracking-widest">{p.name}</p>
                     <p className="text-[6px] lg:text-[8px] text-white/30 font-black uppercase tracking-[0.2em]">{p.rank}</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            {localPlayer.isHost && (
              <div className="flex flex-col items-center gap-4">
                 ...
                 >
                   {canStart ? 'COMBATER' : `HUMANOS: ${humans}/2`}
                 </button>
                 {!canStart && <p className="text-red-400 font-black text-[10px] uppercase animate-pulse">Ranked exige no mínimo 2 jogadores reais!</p>}
              </div>
            )}
          </div>

          <div className="w-full lg:w-96 bg-black/40 backdrop-blur-3xl border-l border-white/10 flex flex-col p-6">
             <h3 className="font-brand text-xl text-blue-400 mb-6 flex items-center gap-3 italic">
               <span className="w-2 h-2 rounded-full bg-emerald-500 animate-ping"></span>
               LENDAS ONLINE
             </h3>
             <div className="flex-1 space-y-3 overflow-y-auto no-scrollbar pr-2">
                {onlinePlayers.filter(p => p.id !== localPlayerId).length === 0 ? (
                  <div className="h-full flex flex-col items-center justify-center opacity-20 text-center gap-4">
                    <span className="text-4xl">📡</span>
                    <p className="font-black uppercase tracking-widest text-[8px]">Escaneando rede...</p>
                  </div>
                ) : (
                  onlinePlayers.filter(p => p.id !== localPlayerId).map(p => (
                    <div key={p.id} className="bg-white/5 p-3 rounded-2xl border border-white/5 flex items-center justify-between hover:bg-white/10 transition-all">
                       <div className="flex items-center gap-3">
                          <div className="w-10 h-10 rounded-full bg-black/40 flex items-center justify-center text-2xl">{p.avatar}</div>
                          <div>
                            <p className="font-bold text-[10px] uppercase tracking-wider">{p.name}</p>
                            <p className="text-[7px] text-yellow-500 font-black uppercase">{p.rank}</p>
                          </div>
                       </div>
                       <button className="px-3 py-1 bg-blue-600 rounded-lg text-[8px] font-black uppercase hover:bg-blue-500 transition-colors">Convidar</button>
                    </div>
                  ))
                )}
             </div>
  ...
  return (
    <div className="flex-1 flex flex-col h-full relative overflow-hidden bg-[#022c22]">
      ...
      <div className="flex-1 flex items-center justify-center relative scale-85 lg:scale-100 z-20">
        <div className="flex items-center gap-10 lg:gap-20 relative">
          ...
          <div className="relative">
            <UnoCard card={topDiscard} size="md" disabled />
      ...
      <div className="h-[45%] flex flex-col items-center justify-end z-[100] relative pb-2">
        <div className="w-full flex flex-col items-center mb-2 pointer-events-none">
           {isMyTurn && <div className="bg-yellow-500 text-emerald-950 px-6 py-1.5 rounded-full font-brand text-xs lg:text-xl shadow-2xl border-2 border-white animate-bounce pointer-events-auto">SUA VEZ!</div>}
        </div>
        ...
        <div className="w-[95%] max-w-4xl bg-black/80 backdrop-blur-3xl rounded-[2.5rem] border border-white/10 p-2 lg:p-3 flex items-center justify-between shadow-2xl mb-4">
           <div className="flex items-center gap-3">
              ...
              <div className="flex gap-2">
                 <div className="relative">
                   <button onClick={() => { setShowEmojiMenu(!showEmojiMenu); setShowVoiceMenu(false); }} className={`w-9 h-9 rounded-full bg-white/5 flex items-center justify-center text-lg transition-all ${showEmojiMenu ? 'bg-white/20' : ''}`}>💬</button>
                   {showEmojiMenu && (
                     <div className="absolute bottom-14 left-0 bg-black/95 backdrop-blur-3xl p-3 rounded-2xl border border-white/20 shadow-2xl z-[500] animate-slide-up w-44">
                        <div className="grid grid-cols-4 gap-2">{ALL_EMOJIS.map(e => <button key={e} onClick={() => { onSendReaction(localPlayerId, e); setShowEmojiMenu(false); }} className="text-2xl p-1 hover:scale-125 transition-transform">{e}</button>)}</div>
                     </div>
                   )}
                 </div>
                 <div className="relative">
                   <button onClick={() => { setShowVoiceMenu(!showVoiceMenu); setShowEmojiMenu(false); }} className={`w-9 h-9 rounded-full bg-white/5 flex items-center justify-center text-lg transition-all ${showVoiceMenu ? 'bg-white/20' : ''}`}>🗣️</button>
                   {showVoiceMenu && (
                     <div className="absolute bottom-14 left-0 bg-black/95 backdrop-blur-3xl p-2 rounded-2xl border border-white/20 shadow-2xl z-[500] animate-slide-up w-64 max-h-[180px] overflow-y-auto no-scrollbar">
                        <div className="flex flex-col gap-1">{VOICE_PHRASES.map(phrase => <button key={phrase} onClick={() => { onSendVoice(localPlayerId, phrase); setShowVoiceMenu(false); }} className="text-left px-3 py-2.5 hover:bg-white/10 rounded-xl text-[7px] font-black uppercase truncate border border-transparent transition-all">{phrase}</button>)}</div>
                     </div>
                   )}
                 </div>
              </div>
           ...
           <div className="flex items-center gap-3">
              ...
              {selectedIds.length > 0 && !showPicker && (
                <div className="flex items-center gap-2">
                  {selectedIds.length > 1 && <span className="bg-blue-500 text-white px-3 py-1 rounded-full font-brand text-xs animate-bounce">COMBO x{selectedIds.length}</span>}
                  <button onClick={() => onPlayCard(localPlayerId, selectedIds, undefined, unoDeclared)} className="px-8 py-3 bg-yellow-500 text-emerald-950 font-brand text-sm lg:text-2xl rounded-full shadow-xl hover:bg-yellow-400 active:scale-95 transition-all">JOGAR</button>
                </div>
              )}
      ...
      {showPicker && (
        <div className="fixed inset-0 bg-black/95 backdrop-blur-3xl z-[600] flex items-center justify-center animate-fade-in p-6">
           <div className="bg-emerald-950/40 p-8 rounded-[3rem] border border-white/10 text-center max-w-sm w-full shadow-2xl">
              <h3 className="text-2xl font-brand text-yellow-400 mb-8 italic uppercase">ESCOLHA A COR</h3>
              <div className="grid grid-cols-2 gap-4">
                {[CardColor.RED, CardColor.BLUE, CardColor.GREEN, CardColor.YELLOW].map(c => <button key={c} onClick={() => { onPlayCard(localPlayerId, selectedIds, c, unoDeclared); setShowPicker(false); }} className={`w-full aspect-square rounded-3xl ${COLOR_CLASSES[c]} border-4 border-white/10 hover:border-white hover:scale-105 transition-all shadow-xl`}></button>)}
              </div>
           </div>
        </div>
      )}
