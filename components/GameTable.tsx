...
const GameTable: React.FC<GameTableProps> = ({ gameState, localPlayerId, equippedSkin, reactions, onPlayCard, onDrawCard, onCallUno, onSendReaction, onTimeout, onSendVoice, onStartGame, onLeaveRoom, onlinePlayers }) => {
  ...
  if (!localPlayer || gameState.players.length === 0) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center bg-[#022c22] gap-4">
        <div className="w-16 h-16 border-4 border-yellow-500/20 border-t-yellow-500 rounded-full animate-spin"></div>
        <p className="font-brand text-yellow-500 animate-pulse uppercase tracking-widest text-xs">Sincronizando com a Arena...</p>
      </div>
    );
  }
        ...
        <div className="flex-1 flex flex-col lg:flex-row h-full animate-fade-in z-10 overflow-hidden relative">
          ...
          <div className="flex-1 flex flex-col items-center justify-center p-6 space-y-8 bg-black/20 text-center">
            <div className="space-y-2">
              ...
              <button onClick={handleCopyCode} className={`text-[10px] font-black uppercase tracking-[0.3em] transition-colors ${copySuccess ? 'text-emerald-400' : 'text-white/30 hover:text-white'}`}>
            ...
            {localPlayer.isHost && (
              ...
              >
                {canStart ? 'COMBATER' : `MÍNIMO 2 JOGADORES`}
              </button>
            )}
          </div>
        </div>
  ...
  return (
    <div className="flex-1 flex flex-col h-full relative overflow-hidden bg-[#022c22]">
      ...
      {showAudioSettings && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-[500] flex items-center justify-center p-6" onClick={() => setShowAudioSettings(false)}>
           <div className="bg-emerald-950 p-8 rounded-[3rem] border border-white/10 w-full max-w-sm" onClick={e => e.stopPropagation()}>
              <h3 className="text-2xl font-brand text-yellow-400 mb-6 italic text-center">CONFIGURAÇÕES</h3>
              <div className="space-y-6">
                 ...
                 <button onClick={() => audio.nextTrack()} className="w-full py-4 bg-white/5 rounded-2xl font-bold uppercase text-[10px] tracking-widest hover:bg-white/10 transition-colors">Trocar Música 🎵</button>
      ...
      <div className="h-[45%] flex flex-col items-center justify-end z-[100] relative pb-2">
        {isMyTurn && <div className="bg-yellow-500 text-emerald-950 px-6 py-1.5 rounded-full font-brand text-xs lg:text-xl shadow-2xl border-2 border-white animate-bounce mb-2 uppercase">Sua Vez</div>}
        <div className="w-[95%] max-w-4xl bg-black/80 backdrop-blur-3xl rounded-[2.5rem] border border-white/10 p-2 lg:p-3 flex items-center justify-between shadow-2xl mb-4">
           <div className="flex items-center gap-3">
              ...
              <div className="flex gap-2">
                 <button onClick={() => setShowEmojiMenu(!showEmojiMenu)} className={`w-9 h-9 rounded-full flex items-center justify-center transition-all ${showEmojiMenu ? 'bg-yellow-400 scale-110' : 'bg-white/5'}`}>💬</button>
                 <button onClick={() => setShowVoiceMenu(!showVoiceMenu)} className={`w-9 h-9 rounded-full flex items-center justify-center transition-all ${showVoiceMenu ? 'bg-blue-500 scale-110' : 'bg-white/5'}`}>🗣️</button>
              </div>
           </div>
           <div className="flex items-center gap-3">
        ...
        <div className="w-full h-32 lg:h-40 overflow-visible">
          <div className="flex justify-center items-end w-full max-w-full overflow-visible px-2 h-full">
            {localPlayer.hand.map((card, i) => {
              ...
              return (
                <div key={card.id} onClick={() => handleCardSelect(card.id)} className={`transition-all duration-200 relative transform-gpu ${isSelected ? '-translate-y-16 z-[100] scale-110' : 'hover:-translate-y-8 z-10'}`} style={{ marginLeft: i === 0 ? 0 : '-40px' }}>
      ...
      {showEmojiMenu && (
        <div className="fixed inset-0 bg-black/40 z-[600]" onClick={() => setShowEmojiMenu(false)}>
          <div className="absolute bottom-40 left-10 bg-[#011a14] p-4 rounded-3xl border border-white/10 grid grid-cols-4 gap-2 animate-slide-up shadow-2xl" onClick={e => e.stopPropagation()}>
      ...
      {showVoiceMenu && (
        <div className="fixed inset-0 bg-black/40 z-[600]" onClick={() => setShowVoiceMenu(false)}>
          <div className="absolute bottom-40 left-10 bg-[#011a14] p-4 rounded-3xl border border-white/10 flex flex-col gap-1 animate-slide-up w-64 max-h-60 overflow-y-auto no-scrollbar shadow-2xl" onClick={e => e.stopPropagation()}>
             {VOICE_PHRASES.map(v => <button key={v} onClick={() => { onSendVoice(localPlayerId, v); setShowVoiceMenu(false); }} className="text-left px-4 py-3 hover:bg-white/10 rounded-xl text-[8px] font-black uppercase tracking-widest border border-white/5 text-white/40">{v}</button>)}
          </div>
        </div>
      )}
